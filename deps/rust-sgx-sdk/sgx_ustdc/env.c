...
int u_getenv_ocall(int *error, const char *name, char *buf, size_t size)
{
    int ret = 0;
    if (buf == NULL || size == 0) {
        if (error) {
    ...
    char *rptr = getenv(name);
    if (rptr != NULL) {
        size_t sn = strlen(rptr) + 1;
        if (sn > size) {
            if (error) {
                *error = ERANGE;
            }
...
int u_getcwd_ocall(int *error, char *buf, size_t size)
{
    int ret = 0;
    if (buf == NULL || size == 0) {
        if (error) {
    ...
    char *rptr = getcwd(buf, size);
    if (rptr == NULL) {
        ret = -1;
    }
    if (error) {
        *error = rptr == NULL ? errno : 0;
    }
    return ret;
}
