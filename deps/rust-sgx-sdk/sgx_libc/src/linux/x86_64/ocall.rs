...
pub unsafe fn getenv(name: *const c_char, buf: *mut c_char, size: size_t) -> c_int {
    let mut result: c_int = 0;
    let mut error: c_int = 0;
...
pub unsafe fn getcwd(buf: *mut c_char, size: size_t) -> c_int {
    let mut result: c_int = 0;
    let mut error: c_int = 0;
...
pub unsafe fn realpath(pathname: *const c_char, resolved_buf: *mut c_char, bufsz: size_t) -> c_int {
    let mut result: c_int = 0;
    let mut error: c_int = 0;
...
pub unsafe fn read(fd: c_int, buf: *mut c_void, count: size_t) -> ssize_t {
    const OCALL_SIZE: usize = 6 * mem::size_of::<usize>();
    let mut result: ssize_t = 0;
    let mut error: c_int = 0;
    ...
    let free_size = ocremain_size();
    if free_size <= OCALL_SIZE {
        set_errno(ENOMEM);
        return -1;
    }

    let tmp_buf = if count <= free_size - OCALL_SIZE {
        used_heap = false;
        let tmp_buf = sgx_ocalloc(count);
    ...
    if result != -1 {
        #[cfg(feature = "hyper")]
        if used_heap {
            let count = cmp::min(count, result.try_into().unwrap_or(0));
            if count > 0 {
                let ret = read_shared_buf(tmp_buf, buf, count, 0);
                if count != ret as usize {
                    result = -1;
                }
            }
        } else {
...
pub unsafe fn pread64(fd: c_int, buf: *mut c_void, count: size_t, offset: off64_t) -> ssize_t {
    const OCALL_SIZE: usize = 7 * mem::size_of::<usize>();
    let mut result: ssize_t = 0;
    let mut error: c_int = 0;
    let mut used_heap = false;
    ...
    if result != -1 {
        ...
        if used_heap {
            let count = cmp::min(count, result.try_into().unwrap_or(0));
            if count > 0 {
                let ret = read_shared_buf(tmp_buf, buf, result as usize, 0);
                if result != ret {
                    result = -1;
                }
            }
...
pub unsafe fn readv(fd: c_int, iov: *const iovec, iovcnt: c_int) -> ssize_t {
    ...
    let iobase = if total_size <= free_size - OCALL_SIZE {
        used_heap = false;
        let tmp_buf = sgx_ocalloc(total_size);
        tmp_buf.write_bytes(0_u8, total_size);
        tmp_buf
    } else {
        used_heap = true;
        malloc(total_size)
    ...
    if result != -1 {
        ...
        for i in 0..v.len() {
            ...
            #[cfg(feature = "hyper")]
            if used_heap {
                let count = cmp::min(v[i].iov_len, remaining_bytes);
                let ret = read_shared_buf(tmpiovec[i].iov_base, v[i].iov_base, count, 0);
                if count != ret as usize {
                    result = -1;
...
pub unsafe fn write(fd: c_int, buf: *const c_void, count: size_t) -> ssize_t {
    ...
    #[cfg(feature = "hyper")]
    if used_heap {
        let ret = write_shared_buf(tmp_buf, buf, count, 0);
        if ret as usize != count {
            free(tmp_buf);
            return -1;
        }
    } else {
...
pub unsafe fn pwrite64(fd: c_int, buf: *const c_void, count: size_t, offset: off64_t) -> ssize_t {
    const OCALL_SIZE: usize = 8 * mem::size_of::<usize>();
    let mut result: ssize_t = 0;
    let mut error: c_int = 0;
    let mut used_heap = false;
...
pub unsafe fn writev(fd: c_int, iov: *const iovec, iovcnt: c_int) -> ssize_t {
    ...
    for io in v {
        ...
        #[cfg(feature = "hyper")]
        if used_heap {
            let ret = write_shared_buf(tmpiov.iov_base, io.iov_base, io.iov_len, 0);
            if ret as usize != io.iov_len {
                free(iobase as *mut c_void);
                return -1;
            }
        } else {
...
pub unsafe fn send(sockfd: c_int, buf: *const c_void, len: size_t, flags: c_int) -> ssize_t {
    ...
    let tmp_buf = if len <= free_size - OCALL_SIZE {
        used_heap = false;
        sgx_ocalloc(len)
    } else {
        used_heap = true;
    ...
    #[cfg(feature = "hyper")]
    if used_heap {
        let ret = write_shared_buf(tmp_buf, buf, len, 0);
        if ret as usize != len {
            free(tmp_buf);
            return -1;
        }
...
pub unsafe fn sendto(
    ...
    let ocall_size = 9 * mem::size_of::<usize>() + addrlen as usize;
    let mut result: ssize_t = 0;
    let mut error: c_int = 0;
    let mut used_heap = false;
...
pub unsafe fn sendmsg(sockfd: c_int, msg: *const msghdr, flags: c_int) -> ssize_t {
    ...
    let io_data: Vec<iovec> = iovecs
        ...
        .map(|v| {
            ...
            #[cfg(feature = "hyper")]
            let _ = write_shared_buf(iov.iov_base, v.iov_base, v.iov_len, 0);
            #[cfg(not(feature = "hyper"))]
...
pub unsafe fn recv(sockfd: c_int, buf: *mut c_void, len: size_t, flags: c_int) -> ssize_t {
    ...
    if result != -1 {
        ...
        if used_heap {
            ...
            ptr::copy_nonoverlapping(tmp_buf as *const u8, buf as *mut u8, result as usize);
        }
        #[cfg(not(feature = "hyper"))]
        ptr::copy_nonoverlapping(tmp_buf as *const u8, buf as *mut u8, result as usize);
    }
...
pub unsafe fn recvfrom(
    ...
    let len_in: socklen_t = if !addrlen.is_null() { *addrlen } else { 0 };
    let mut len_out: socklen_t = 0 as socklen_t;
    let ocall_size = 11 * mem::size_of::<usize>() + len_in as usize;
    let mut used_heap = false;
...
pub unsafe fn recvmsg(sockfd: c_int, msg: *mut msghdr, flags: c_int) -> ssize_t {
    ...
    if result >= 0 {
        ...
        for i in 0..iovecs.len() {
            ...
            #[cfg(feature = "hyper")]
            {
                let ret = read_shared_buf(io_data[i].iov_base, iovecs[i].iov_base, copy_len, 0);
                if copy_len != ret as usize {
                    result = -1;
                    free(io_base as *mut c_void);
                    return result;
                }
        ...
        if result == 0 {
            *res = ptr::null_mut();
            ...
            while cur_ptr != ptr::null_mut() {
                ...
                #[cfg(feature = "hyper")]
                let cur = {
                    let mut cur: addrinfo = mem::zeroed();
                    let ret = read_shared_buf(
                        cur_ptr as *const _,
                        &mut cur as *mut _ as *mut _,
                        mem::size_of::<addrinfo>(),
                        0,
                    );
                    if ret as usize != mem::size_of::<addrinfo>() {
                        break;
                    }
                    cur
                };
                ...
                if !cur.ai_addr.is_null() && cur.ai_addrlen > 0 {
                    ...
                    #[cfg(feature = "hyper")]
                    let ret_size = read_shared_buf(
                        cur.ai_addr as *const _,
                        addr_vec.as_mut_ptr() as *mut _,
                        cur.ai_addrlen as usize,
                        0,
                    ) as usize;
                    #[cfg(not(feature = "hyper"))]
                    let ret_size = {
                        let addr_slice: &[u8] = slice::from_raw_parts(
                    ...
                    if ret_size == cur.ai_addrlen as usize {
...
fn ocremain_size() -> usize {
    #[cfg(feature = "hyper")]
    return unsafe { sgx_ocremain_size() };
    #[cfg(not(feature = "hyper"))]
    return MAX_OCALL_ALLOC_SIZE;
}

#[cfg(feature = "hyper")]
pub unsafe fn read_shared_buf(
    base: *const c_void,
    data: *mut c_void,
    count: size_t,
    offset: off64_t,
    ...
    let mut result: c_int = 0;
    let mut left_len = count;
    let mut per_offset = 0_usize;

    let free_size = shared_buf_free_size();
    if free_size == 0 {
        set_errno(ENOMEM);
        return -1;
    }
    ...
    while left_len > 0 {
        let copy_len = core::cmp::min(left_len, free_size);
        let src = (base as *const u8).add(offset as usize);
        let dest = (data as *mut u8).add(per_offset);

        let status = u_read_shared_buf(
            &mut result as *mut c_int,
            src as *const c_void,
            dest as *mut c_void,
            copy_len,
            per_offset as off64_t,
        );
        if status == sgx_status_t::SGX_SUCCESS {
            if result != 0 {
                set_errno(result);
                return (count - left_len) as ssize_t;
            }
        } else {
            set_errno(ESGX);
            return (count - left_len) as ssize_t;
...
#[cfg(feature = "hyper")]
pub unsafe fn write_shared_buf(
    base: *mut c_void,
    data: *const c_void,
    count: size_t,
    offset: off64_t,
    ...
    while left_len > 0 {
        ...
        let src = (data as *mut u8).add(per_offset);

        let status = u_write_shared_buf(
            &mut result as *mut c_int,
            dest as *mut c_void,
            src as *const c_void,
            copy_len,
...
#[cfg(feature = "hyper")]
fn shared_buf_free_size() -> size_t {
    const OCALL_SIZE: usize = 8 * mem::size_of::<usize>();

    let free_size = unsafe { sgx_ocremain_size() };
    if free_size > OCALL_SIZE {
        free_size - OCALL_SIZE
    } else {
        0
    }
}
