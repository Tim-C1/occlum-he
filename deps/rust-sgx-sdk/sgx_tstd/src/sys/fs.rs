...
use sgx_libc::{c_char, c_int, dirent64, mode_t, off64_t, stat64};
...
pub fn canonicalize(p: &Path) -> io::Result<PathBuf> {
    ...
    let bufsz = libc::PATH_MAX as usize;
    let mut buf = vec![0_u8; bufsz];

    unsafe {
        cvt(libc::realpath(path.as_ptr(), buf.as_mut_ptr() as *mut c_char, bufsz))?;
