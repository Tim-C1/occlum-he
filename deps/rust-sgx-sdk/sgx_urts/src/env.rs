...
pub extern "C" fn u_getenv_ocall(
    ...
    let mut errno = 0;
    let rptr = unsafe { libc::getenv(name) };
    let ret = if !rptr.is_null() {
        let sn = unsafe { libc::strlen(rptr) + 1 };
        if sn > size {
            errno = libc::ERANGE;
            -1
        } else {
            unsafe { ptr::copy_nonoverlapping(rptr, buf, sn) };
            0
        }  
    } else {
        unsafe { *buf = 0 };
...
pub extern "C" fn u_getcwd_ocall(error: *mut c_int, buf: *mut c_char, size: size_t) -> c_int {
    ...
    let mut errno = 0;
    let rptr = unsafe { libc::getcwd(buf, size) };
    let ret = if rptr.is_null() {
        errno = Error::last_os_error().raw_os_error().unwrap_or(0);
        -1
    } else {
